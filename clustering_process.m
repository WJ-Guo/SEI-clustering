...
tic;
data = Unbalance(:,1:(end-1));

%% centralize and scale the data
data = data - repmat(mean(data),size(data,1),1);
data = data/max(max(abs(data)));

true_label = Unbalance(:,end);

halo = true_label;
NClusters = max(halo);
% twoUnbalance = [[two_Unbalance(xx1,[1,2]),yy1];[two_Unbalance(xx2,[1,2]),-yy2]];
...
knn = 20 + floor(num/200)
k_rate = 0.3;
find_k = floor(knn * k_rate)
...
save UnbalanceDPCmdistance.distance   mdistance -ascii                
                
%% load diatance matrix if exists				
load UnbalanceDPCmdistance.distance;	
xx = UnbalanceDPCmdistance;
...
set(gca,'FontName','Times New Roman','FontSize',14)
title ('Decision Graph of Unbalance','FontSize',17.0)
box off;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  FMI,ARI,NMI  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    fprintf('FMI value of DPC on Unbalance dataset %i \n', DPCFMI);
    [cluster_acc,randindex,DPCARI] = ARI(true_label(:),halo(:));
    fprintf('ARI value of DPC on Unbalance dataset %i \n', DPCARI);
    DPCNMI = NMI(true_label(:),halo(:));
    fprintf('NMI value of DPC on Unbalance dataset %i \n', DPCNMI);
